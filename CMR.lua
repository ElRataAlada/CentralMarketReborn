...
function parseInventoryItems(text, title)
    ...
    for n in text:gmatch('[^\r\n]+') do -- чек построчно 
        ...
        if n:find(">>") then wait(parserBuf.v) sampSendDialogResponse(parseListforsale, 1, i-1) isNext = true end -- следующая страничка
        i = i + 1
    end
...
function parserPage(text, title) -- переделал функу Devilov'a
    ...
    for n in text:gmatch('[^\r\n]+') do -- чек построчно 
		...
		if n:find(">>>") and (cur ~= max) then wait(parserBuf.v) sampSendDialogResponse(parseItemsForBuy, 1, i-1) isNext = true end -- следующая страничка
        i = i + 1
	end
    
    if not isNext then check = false sampAddChatMessage('[ Central Market Reborn ]: {FFFFFF}Проверка списков прошла успешно! Откройте меню по команде: {'..settings.main.color..'}/cmr', settings.main.colormsg) jsonSave(json_file_BuyList, itemsBuy) sampSendDialogResponse(parseItemsForBuy, 0) skip = false end
end
imgui.Scroller = {
	_ids = {},
	_version = 1,
